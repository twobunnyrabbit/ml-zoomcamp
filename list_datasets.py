            ...
            with open('.env', 'r') as f:
                for line in f:
                    ...
                    if line.startswith('DATASET_PATH='):
                        ...
                        if (dataset_path.startswith("'") and dataset_path.endswith("'")) or \
...
def find_dataset_files(directory: str, extensions: List[str], recursive: bool = True) -> List[str]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="List dataset files in the specified directory")
    
    parser.add_argument(
        "--path", 
        help="Custom path to search for datasets (overrides .env file)"
    )
